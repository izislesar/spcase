...
const listJuryTeamsQuery = `
	WITH member_agg AS (
		SELECT team_id, COUNT(*) AS members_count
		FROM team_members
		GROUP BY team_id
	),
	jury_evaluation_agg AS (
		SELECT team_id, COUNT(*) AS criteria_count
		FROM evaluations
		WHERE jury_id = $1
		GROUP BY team_id
	)
	SELECT
		...
		t.name,
		s.solution_url,
		COALESCE(je.criteria_count, 0) = $2,
		COALESCE(m.members_count, 0),
		s.updated_at
	FROM submissions s
	JOIN teams t ON t.id = s.team_id
	LEFT JOIN member_agg m ON m.team_id = t.id
	LEFT JOIN jury_evaluation_agg je ON je.team_id = t.id
	ORDER BY t.name
`

const exportSummaryQuery = `
	WITH member_agg AS (
...
func (r *QueryPostgres) ListJuryTeams(ctx context.Context, juryID uuid.UUID) ([]domain.JuryTeam, error) {
	rows, err := r.pool.Query(ctx, listJuryTeamsQuery, juryID, domain.CriterionCount)
	if err != nil {
...
func (r *QueryPostgres) ExportSummary(ctx context.Context) ([]domain.ExportSummaryRow, error) {
	rows, err := r.pool.Query(ctx, exportSummaryQuery)
	if err != nil {
...
func (r *QueryPostgres) ExportDetails(ctx context.Context) ([]domain.ExportDetailRow, error) {
	rows, err := r.pool.Query(ctx, exportDetailsQuery)
	if err != nil {
